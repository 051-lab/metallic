...
(() => {
  ...
  var list = document.querySelector("#platformList");
  var toast = document.querySelector("#toast");
  var persistentSiteList = document.querySelector("#persistentSiteList");
  ...
  async function render() {
    ...
    for (const platform of PLATFORMS) {
      ...
      checkbox.checked = enabledPlatforms.includes(platform.id) && platform.origins.every((origin) => permissions.origins?.includes(origin));
      ...
      checkbox.addEventListener("change", async () => {
        ...
        if (checkbox.checked) {
          ...
          next = [.../* @__PURE__ */ new Set([...next, platform.id])];
        } else {
          await chrome.permissions.remove({ origins: platform.origins });
          next = next.filter((id) => id !== platform.id);
        }
  ...
  async function renderPersistentSites() {
    ...
    for (const origin of persistentOrigins) {
  ...
  document.addEventListener("DOMContentLoaded", () => Promise.all([render(), renderPersistentSites(), renderProfiles()]));
  document.querySelector("#preferenceForm")?.addEventListener("submit", async (event) => {
    event.preventDefault();
    await chrome.storage.sync.set({
      launcherEnabled: document.querySelector("#launcherEnabled").checked,
      launcherPosition: document.querySelector("#launcherPosition").value,
      filenameTemplate: document.querySelector("#filenameTemplate").value.trim() || "{platform}-{title}"
    });
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.querySelector("#launcherEnabled").checked = settings.launcherEnabled;
    document.querySelector("#launcherPosition").value = settings.launcherPosition;
    document.querySelector("#filenameTemplate").value = settings.filenameTemplate;
  });
  ...
  document.querySelector("#importProfiles")?.addEventListener("click", () => document.querySelector("#profileFile").click());
  document.querySelector("#profileFile")?.addEventListener("change", async (event) => {
    ...
    try {
      ...
      const conflicts = values.filter(
        (value) => value && typeof value === "object" && currentIds.has(value.id || "")
      ).length;
