...
// Function to convert structured chat data to Markdown with YAML frontmatter
async function convertHtmlToMarkdown(extractedData) {
    try {
        console.log('Converting Structured Data to Markdown...');
        ...
        // Add custom rule to preserve citations
        turndownService.addRule('citations', {
            filter: function (node, options) {
                return (node.nodeName === 'A' || node.nodeName === 'SUP') &&
                       (node.classList.contains('citation') || /^\[\d+\]$/.test(node.textContent.trim()));
            },
        ...
        if (extractedData.notebookContext) {
            markdown += `notebook: ${JSON.stringify(extractedData.notebookContext)}\n`;
        }
        markdown += '---\n\n';

        extractedData.blocks.forEach(block => {
            const converted = turndownService.turndown(block.html);
            if (block.role === 'user') {
                markdown += `**User:**\n${converted}\n\n`;
            } else {
                markdown += `**Gemini:**\n${converted}\n\n`;
        ...
        console.log('Successfully converted to Markdown with Frontmatter');
        return markdown;
    } catch (error) {
...
// Function to convert structured chat data to Jupyter Notebook (.ipynb)
async function convertHtmlToJupyter(extractedData) {
    try {
        console.log('Converting Structured Data to Jupyter Notebook...');
        const turndownService = new TurndownService({
            headingStyle: 'atx',
            hr: '---',
            bulletListMarker: '*',
        ...
        // Metadata Markdown Cell
        let metaSource = `# Gemini Conversation\n**Date:** ${new Date().toLocaleString()}\n`;
        if (extractedData.notebookContext) {
            metaSource += `**Notebook Context:** ${extractedData.notebookContext}\n`;
        }
        cells.push({
            cell_type: "markdown",
            metadata: {},
        ...
        const appendMarkdownCell = (source) => {
        ...
        extractedData.blocks.forEach(block => {
            const converted = turndownService.turndown(block.html);
            const speaker = block.role === 'user' ? 'User' : 'Gemini';
            const codeBlockRegex = /```([^\n`]*)\n([\s\S]*?)```/g;
            let lastIndex = 0;
            let match;

            appendMarkdownCell(`**${speaker}:**`);

            while ((match = codeBlockRegex.exec(converted)) !== null) {
                appendMarkdownCell(converted.substring(lastIndex, match.index));
            ...
            appendMarkdownCell(converted.substring(lastIndex));
        });
        ...
        console.log('Successfully converted to Jupyter Notebook');
        return JSON.stringify(notebook, null, 2);
    } catch (error) {
        console.error('Error converting to Jupyter Notebook:', error);
        showToast('Error converting to Jupyter Notebook', 'error');
        return null;
    }
}
...
function detectNotebookContext() {
    ...
    // those can silently mislabel an ordinary conversation as a notebook.
    const selectors = [
        '[data-test-id="notebook-title"]',
        '[data-testid="notebook-title"]',
        '[aria-label^="Notebook:"]',
        'a[href*="/notebook/"][aria-current="page"]',
        'a[href*="/notebooks/"][aria-current="page"]'
    ];

    for (const selector of selectors) {
        const element = document.querySelector(selector);
        const ariaLabel = element?.getAttribute('aria-label') || '';
        const title = (element?.textContent || ariaLabel.replace(/^Notebook:\s*/i, '')).trim();
...
// Function to extract structured chat blocks with markers
function extractChatHtmlWithMarkers() {
    try {
        ...
        let allChatBubbles = Array.from(chatHistoryContainer.querySelectorAll(
            'user-query-content, bot-response-content, model-response'
        ));

        // Fall back to conversation containers only when semantic message
        // elements are unavailable; mixing both produces duplicate content.
        if (allChatBubbles.length === 0) {
            allChatBubbles = Array.from(
                chatHistoryContainer.querySelectorAll('.conversation-container')
            );
        }

        if (allChatBubbles.length === 0) {
            console.warn('No chat message bubbles found.');
        ...
        allChatBubbles.forEach((element) => {
            const isUser =
                element.matches('user-query-content, .user-query-content') ||
                element.querySelector('user-query-content, .user-query-content') !== null ||
                /user/i.test(element.getAttribute('data-message-author-role') || '');
        ...
        const notebookContext = detectNotebookContext();

        console.log('Structured chat extracted successfully!');
        return {
            blocks: chatBlocks,
...
function showUtilitiesOverlay(event) {
    ...
    // Utility option 1.5: Download as Jupyter Notebook
    const jupyterOption = createUtilityOption(
        '📓',
        'Export as .ipynb',
        'Extract code blocks to a Jupyter Notebook',
        async () => {
            overlay.remove();
            const extractedData = extractChatHtmlWithMarkers();
            if (extractedData) {
                const jupyterContent = await convertHtmlToJupyter(extractedData);
                if (jupyterContent) {
                    const h2Elements = document.querySelectorAll('h2[data-sourcepos]');
                    const h2Element = h2Elements[h2Elements.length - 1];
                    let filename = h2Element ? h2Element.textContent.trim() : 'Gemini_Notebook_' + new Date().getTime();
                    filename = filename.replace(/[<>:"/\\|?*\u0000-\u001F]/g, '_').trim() || 'Gemini_Notebook';

                    const blob = new Blob([jupyterContent], { type: 'application/x-ipynb+json' });
                    const url = URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
                    ...
                    showToast('Jupyter Notebook downloaded successfully!');
                }
            }
        }
    );
    ...
    // Utility option 5: Save to Archive
    const archiveOption = createUtilityOption(
        '🗄️',
        'Save to Archive',
        'Silently save conversation to local archive',
        async () => {
            try {
                const extractedHtml = extractChatHtmlWithMarkers();
                if (extractedHtml) {
                    const markdownContent = await convertHtmlToMarkdown(extractedHtml);
                    if (markdownContent && window.ArchiveDB) {
                        const h2Elements = document.querySelectorAll('h2[data-sourcepos]');
                        const h2Element = h2Elements[h2Elements.length - 1];
                        const title = h2Element ? h2Element.textContent.trim() : 'Gemini Conversation ' + new Date().toLocaleString();
                        await window.ArchiveDB.saveConversation(title, markdownContent, extractedHtml.notebookContext);
    ...
    utilitiesGrid.appendChild(jupyterOption);
    utilitiesGrid.appendChild(copyOption);
    utilitiesGrid.appendChild(clearOption);
    utilitiesGrid.appendChild(archiveOption);
...
if (!window.geminiToMarkdownInjected) {
...
if (!window.geminiToMarkdownMessageListenerAdded) {
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        ...
        if (request.action === 'showOverlay') {
            console.log('[GeminiNoteTaker] Showing overlay via message command.');
            showUtilitiesOverlay();
            sendResponse({ ok: true });
        }
    });
    window.geminiToMarkdownMessageListenerAdded = true;
}
