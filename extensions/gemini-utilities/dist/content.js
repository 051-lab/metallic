...
(() => {
  ...
  var profile = (id, name, origin, messages = commonMessages) => ({
    schemaVersion: 1,
    id,
    name,
    source: "bundled",
...
`;
  ...
  function showOverlay(adapter, snapshot, onAction, detection) {
    ...
    <header><div><div class="eyebrow">${escapeHtml(adapter.displayName)}</div><h2>AI Chat Utilities</h2>
      <p>${snapshot ? `${snapshot.messages.length} messages \xB7 ${escapeHtml(snapshot.title)}` : "No conversation detected yet."}</p></div>
      <button class="close" aria-label="Close">\xD7</button></header>
    <div class="status ${warning ? "warning" : ""}">${warning ? escapeHtml(warning) : `Conversation ready \xB7 ${Math.round((detection?.confidence || 1) * 100)}% extraction confidence.`}</div>
    ${snapshot ? `<div class="preview">${snapshot.messages.slice(0, 6).map((message) => `<div class="message"><b>${message.role}</b><span></span></div>`).join("")}</div>` : ""}
    <div class="grid">
